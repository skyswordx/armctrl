...
- Added `scripts/x5_oed_followup_plan.py` to turn a frozen Fourier candidate into explicit replay and structural OED scan commands over duration, waypoint count, stack repetitions, amplitude, and a small representative seed set, while documenting that true FIGAROH warm-start from CSV is not yet exposed by the current wrapper.
...
- Added a host contract to Fourier OED follow-up plans: FIGAROH/IPOPT structural scans run on local WSL/workstation hosts, while the memory-constrained n100d host is reserved for lightweight replay, hardware collection, postprocess, and solver stages.
- Added `armctrl sysid review-candidate` plus `armctrl.sysid_review` so frozen Fourier OED candidates can be reviewed offline without SDK motion. The review writes q/dq/ddq SVG plots, shared simulation/FK preview artifacts, `review_report.md`, `review_summary.json`, and a non-executing `hardware_smoke_plan.md` while keeping `movement_allowed=false` and `hardware_execution_eligible=false`.
...
- Updated OED scan, freeze, and follow-up target summaries so SysID acceptance is judged by FIGAROH base-regressor condition when available; Pinocchio effective condition is retained as an auxiliary diagnostic only.
...
- WSL evidence from `runs/oed-scan-fourier-accel30-focused-200-20260606` found the current best Fourier candidate at `duration=1.0`, `amplitude=0.5`, `n_wps=5`, `stack_reps=1`, `seed=3`, and `ipopt_max_iterations=200`: safety gates all pass, rank is `36`, Pinocchio effective condition is about `106.28`, FIGAROH base-regressor condition is about `150.84`, execution max step is about `0.0196 rad`, max velocity about `1.95 rad/s`, and max acceleration about `25.51 rad/s^2`. The remaining OED quality gap is FIGAROH base condition above the `<100` SysID target plus optimizer convergence labeling (`optimizer_dual_infeasible`), not execution safety.
...
- WSL evidence from `runs/x5-fourier-local-refine-foreground-20260606` tested a narrow Fourier refinement around the current best (`amplitude=0.49/0.50`, `seed=2/3`, `n_wps=5`, `stack_reps=1`, `ipopt_max_iterations=500`); all four attempts ended as `optimizer_dual_infeasible`, so simply increasing IPOPT iterations is not the path to the `<100` target.
- WSL evidence from `runs/x5-fourier-seed-basin-200-20260606` expanded the current best basin at `amplitude=0.5`, `n_wps=5`, `stack_reps=1`, `ipopt_max_iterations=200` across seeds `4/5/6/7/8/9/11/13`; only seed `4` produced a safe numeric candidate and it was much worse (`condition ~= 258.40`), so the earlier seed `3` candidate at `condition ~= 106.28` remains the best known Fourier OED trajectory.
- WSL evidence from the clean `7bb4c40` snapshot under `/home/circlemoon/armctrl-clean-oed-structural-20260606` ran the requested structural Fourier OED grid exactly as A-D (`duration=2,n_wps=7,stack=1`; `duration=2,n_wps=9,stack=1`; `duration=3,n_wps=9,stack=1`; `duration=2,n_wps=7,stack=2`) over `amplitude=0.45/0.50/0.55` and seeds `2/3/4`. All 36 attempts faulted before producing candidate trajectories: FIGAROH reported missing `T_F/P_F` segments and stderr repeatedly showed `FAILED to generate a feasible cubic spline` with joint velocity-limit violations. This means the next bottleneck is FIGAROH cubic-spline feasibility / waypoint initialization under the current OED velocity limits, not seed count, IPOPT iteration count, execution safety gate, or regressor condition threshold.
- Added an X5 FIGAROH wrapper initialization adapter that first tries deterministic velocity-feasible waypoints bounded by the configured OED velocity limits and waypoint duration, then falls back to FIGAROH's random initializer if the mature backend still rejects the spline. The OED result now records the initialization strategy so structural scan reports can distinguish armctrl initialization fixes from backend optimizer outcomes.
- WSL smoke evidence from `/home/circlemoon/armctrl-clean-oed-structural-code-ed9ce27/runs/oed-structural-init-smoke-20260606` reran `duration=2.0,n_wps=7,stack=1,amplitude=0.45,seed=2` after the initialization adapter. The failure moved past the previous cubic-spline feasibility warning and now stops at `cyipopt is required for IPOPT optimization`, so the remaining smoke blocker is the WSL IPOPT dependency, not the earlier waypoint initialization dead end. OED scan classification now reports this as `figaroh_ipopt_dependency_missing`.
- Added an X5 FIGAROH constraint-contract adapter for the mature backend's waypoint-position constraint mismatch: FIGAROH declared `(n_wps-1)*dof` waypoint position bounds but could omit those values when waypoint times did not exactly match sampled spline times. WSL smoke from `/home/circlemoon/armctrl-clean-oed-structural-code-ed9ce27/runs/oed-structural-init-smoke-contract-it120-20260606` now reaches normal IPOPT iterations with `duration=2.0,n_wps=7,amplitude=0.45,seed=2`, safety gate pass, rank `36`, FIGAROH base condition `76.87`, Pinocchio effective condition `66.58`, max step `0.01848 rad`, max velocity `1.84 rad/s`, and max acceleration `17.82 rad/s^2`; the remaining label is `max_iterations_exceeded` with zero constraint violation, not a Jacobian/cubic-spline interface fault.
- Updated trajectory manifests to use FIGAROH base-regressor condition as the primary `condition_number` whenever the external OED backend reports it, keeping Pinocchio effective condition as an auxiliary diagnostic. WSL smoke from `runs/oed-structural-metric-contract-20260606` records `condition_metric=figaroh_base_regressor`, FIGAROH base condition `68.43`, Pinocchio effective condition `77.57`, rank `36`, safety pass, max step `0.01848 rad`, max velocity `1.84 rad/s`, and max acceleration `17.81 rad/s^2`.
- Tightened frozen Fourier candidate handoff gates: a candidate with FIGAROH base condition `<100` but failed OED quality/convergence now freezes with `next_gate=review_optimizer_convergence_offline`, not `ready_for_hardware_smoke`. The WSL frozen artifact `runs/x5-fourier-best-candidate-metric-contract-safe-freeze-20260606` demonstrates this safe handoff (`target_condition_status=pass`, `oed_quality_status=fail`, `safety_allowed=true`).
- WSL offline review evidence from `/home/circlemoon/armctrl-clean-oed-structural-code-ed9ce27/runs/x5-fourier-best-candidate-metric-contract-safe-freeze-20260606/offline_review` promotes the 68.43 Fourier candidate only to `hardware_smoke_plan_ready`: the reviewed `source_execution_trajectory` has 196 samples, max step `0.018476 rad`, max velocity `1.84275 rad/s`, max acceleration `17.8125 rad/s^2`, FK/table clearance pass with minimum EEF clearance about `0.0921 m`, and FIGAROH base condition `68.43`. The freeze-level `recommended_candidate.csv` remains the 20 Hz planned trajectory, so offline review must use `source_execution_trajectory` for execution gates.
