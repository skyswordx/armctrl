...
如果 EEF command 声明了 adapter，但长驻 runtime 没有配置对应 `eef_backends`，serve 端必须 rejected。允许的失败形态是“缺少 mature backend adapter”；不允许退化为 heuristic joint fallback，也不允许为了执行 EEF command 自动关闭/重开 SDK/CAN。
